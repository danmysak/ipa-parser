...
| `brackets`      | [`BracketStrategy`](#BracketStrategy)<br>`str` | [`BracketStrategy.KEEP`](#BracketStrategy)<br>`'keep'` | What to do with content in brackets denoting optional pronunciation, such as in `[bə(j)ɪz⁽ʲ⁾ˈlʲivɨj]`:<ul><li>`keep` (and treat brackets as invalid IPA characters),</li><li>`expand`: `[bəjɪzʲˈlʲivɨj]`,</li><li>`strip`: `[bəɪzˈlʲivɨj]`.</li></ul> |
